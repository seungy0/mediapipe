...
namespace mediapipe {
    PoseEmbedder::PoseEmbedder(float torso_size_multiplier) : _torso_size_multiplier(
    ...
    PoseEmbedder::normalize_pose_landmarks(const NormalizedLandmarkList &landmarks) {
    ...
    float PoseEmbedder::get_pose_size(const NormalizedLandmarkList &landmarks,
        ...
        float torso_size = sqrt(
                pow(shoulders.x() - hips.x(), 2) + pow(shoulders.y() - hips.y(), 2));
        NormalizedLandmark pose_center = get_pose_center(landmarks);
        float max_dist = 0;
        for (const auto &landmark: landmarks.landmark()) {
            float dist = sqrt(pow(landmark.x() - pose_center.x(), 2) +
                              pow(landmark.y() - pose_center.y(), 2) +
                              pow(landmark.z() - pose_center.z(), 2));
            if (dist > max_dist) {
                max_dist = dist;
            }
        }
    ...
    PoseEmbedder::get_pose_distance_embedding(const NormalizedLandmarkList &landmarks) {
        std::vector <std::vector<float>> embedding;
        embedding.push_back(get_distance(get_average_by_names(landmarks, "left_hip", "right_hip"),
                                         get_average_by_names(landmarks, "left_shoulder",
                                                              "right_shoulder")));
    ...
    NormalizedLandmark PoseEmbedder::get_average_by_names(const NormalizedLandmarkList &landmarks,
                                                          const std::string &name_from,
                                                          const std::string &name_to) {
        const NormalizedLandmark &lmk_from = landmarks.landmark(
                std::find(_landmark_names.begin(), _landmark_names.end(), name_from) -
                _landmark_names.begin());
        const NormalizedLandmark &lmk_to = landmarks.landmark(
                std::find(_landmark_names.begin(), _landmark_names.end(), name_to) -
                _landmark_names.begin());
    ...
    std::vector<float> PoseEmbedder::get_distance_by_names(const NormalizedLandmarkList &landmarks,
                                                           const std::string &name_from,
                                                           const std::string &name_to) {
        const NormalizedLandmark &lmk_from = landmarks.landmark(
                std::find(_landmark_names.begin(), _landmark_names.end(), name_from) -
    ...
    std::vector<float> PoseEmbedder::get_distance(const NormalizedLandmark &lmk_from,
                                                  const NormalizedLandmark &lmk_to) {
